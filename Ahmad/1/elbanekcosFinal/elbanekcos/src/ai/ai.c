...
void find_solution(sokoban_t *init_data, bool show_solution)
{
	...
	mainWindow = newwin(init_data->.lines, cols, 0, 0);
	...
	while (pq.size != 0)
	{
		...
		for (move_t action = left; action <= down; action++)
		{
			...
			bool player_moved = applyAction(init_data, n, &new_node, action);
			generated_nodes++;
			...
			if (!player_moved || simple_corner_deadlock(init_data, &new_node->state))
			{
				...
				continue;
						...
						// char **faltted_map = new char*[init_data->num_char_map]
			flatten_map(init_data, &flat_map, n->state.map);

			heap_display(&pq, init_data);
			print_map(init_data, new_node->state);
			...
			{
				ht_insert(&hashTable, flat_map, p);
