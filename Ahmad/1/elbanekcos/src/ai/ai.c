...
typedef struct
{
	int x;		   // The x-coordinate of the state.
	int y;		   // The y-coordinate of the state.
	int cost;	   // The cost of reaching this state from the initial state.
	int heuristic; // The estimated cost of reaching the goal state from this state.
} state_t;
...
{
	// TODO: Implement a better heuristic function here.
...
state_t *solve(state_t *init_state, state_t *goal_state)
{
	...
	priority_queue_t *queue = priority_queue_create(compare_states);

	// Add the initial state to the priority queue.
	priority_queue_push(queue, init_state);
	...
	while (!priority_queue_empty(queue))
	{
		...
		state_t *current_state = priority_queue_pop(queue);
		...
		for (int i = 0; i < num_successor_states; i++)
		{
			...
			priority_queue_push(queue, successor_states[i]);
		}
	}
